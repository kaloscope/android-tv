...
for width_height in 1280x720 1920x1080 3840x2160; do
    ...
    mkdir -p app/src/androidTest/assets/goldens
    adb -s "$serial" pull \
        /sdcard/Android/data/org.kaloscope.tv/files/goldens/. \
        app/src/androidTest/assets/goldens/ >/dev/null
done

printf '%s\n' "Saved display state: $original_size; $original_density"
